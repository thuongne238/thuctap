...
(function ($, ko) {
    var jqElement = function (element) {
        var jq = $(element);
        if ($(document).find(element).length == 0) {  //The processing element is executed on the parent page
        ...
        var customBinding = {
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                ...
                opt[o.event] = function () {
                    ...
                    if (valueAccessor() == null) throw "viewModel There is no page binding field";
                    valueAccessor()(value);
                };
        ...
        var gridBinding = {
            update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
                ...
                var handler = function () {
                    if (grid.data(gridType)) {
                        ...
                        var defaults = {
                            ...
                            nowrap: true,           //Discount line
                            rownumbers: true,       //Line number
                            striped: true,          //Interlacing discoloration
                            singleSelect: true,     //Radio
                            remoteSort: true,       //Background sorting
                            pagination: false,      //Flip pages
                ...
                grid[gridType] ? handler() : using([gridType], function () {
                    handler();
                    if (gridType == 'treegrid') com.loadCss('/Content/css/icon/icon.css', document, true);//Solve the problem of icon.covered by tree.css
                });
            }
        };
