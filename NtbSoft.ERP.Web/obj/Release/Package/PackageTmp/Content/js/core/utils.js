...
/**
* Format the time display mode
* usage:format="yyyy-MM-dd hh:mm:ss";
*/
utils.formatDate = function (v, format) {
    ...
    if (typeof v === 'string') {
        if (v.indexOf("/Date(") > -1)
            ...
            d = new Date(Date.parse(v.replace(/-/g, "/").replace("T", " ").split(".")[0]));//.split(".")[0] Used to deal with the case of the emergence of ms, truncated .xxx, otherwise it will be wrong
    }
    var o = {
...
/** 
* json Format to tree structure 
* @param   {json}      json data
* @param   {String}    id string
* @param   {String}    Parent id string
* @param   {String}    string for children
* @return  {Array}     Array 
*/
utils.toTreeData = function (a, idStr, pidStr, childrenStr) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (a[i].MenuId != null && a[i].MenuId!='') {
            a[i].id = a[i].MenuId;
            if (a[i].MenuName != null) {
                a[i].name = a[i].MenuName;
                a[i].text = a[i].MenuName;
            }
            a[i].IconClass = a[i].iconCls;
            a[i].attributes = { url: a[i].Url };
            if (a[i].Url == null || a[i].Url == '') {
                a[i].state = "closed";
                if (a[i].iconCls == null || a[i].iconCls == '')
                    a[i].iconCls = 'icon-application';
            } else if (a[i].iconCls == null || a[i].iconCls == '')
                a[i].iconCls = 'icon-page_red';
        }
...
utils.toTreeData2 = function (a, idStr, pidStr, childrenStr) {
    var r = [], hash = {}, len = (a || []).length;
    for (var i = 0; i < len; i++) {
        hash[a[i][idStr]] = a[i];
    }
