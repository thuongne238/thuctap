...
async function GetLenhChiTiet() {

    const maLenhDK = $("#malenhdangky").val()
    const data = await API.Get("GetChiTietLenh", { para1: isNPL, para2: maLenhDK })
...
//Ngăn người dùng click nhiều lần khi đang submit dữ liệu (tránh gọi API trùng)
                ...
                cellTemplate: function (container, options) {
                    const $input = $(`
                        <input type="number"
                                ...
                                value="${parseFloat(parseFloat(options.data.SLDK).toFixed(4)) ?? ''}" />
                    `);
                    ...
                    $input.on("input", function () {
                        ...
                        if (Number(value) > options.data.SLCLDK) {
                            showToast("warning", "Số lượng đăng ký không được lớn hơn số lượng còn lại");
