...
async function CheckBarCode(barcodeValue) {
    if ($("#cbxThungLe").is(":checked")) {
        ...
        if (itemCheckBarCode) {
            ...
            const dataBarCodeN = barcodeN.filter(x => x.MaDH == madh && x.POID == poid && x.DauSizeID == dausizeid && x.ColorID == colorid && x.SizeID == sizeid && x.IsThungLe == isThungLe && x.IsDongThung == 1)

            const dataCDT = dataBarCodeN.filter(x => x.IsNhapKho != 1)
            if (dataCDT.length == 0) {
                iziToast.warning({
                    title: 'Warning',
                    message: 'Thùng đã được nhập hết.',
                    position: 'topRight'
                });
                PlayAudioError()
...
const PostSave = async (arrPost) => {
    PlayAudio();
    const request = new Request(`/api/ScanBarcodeDT_NK/PostWeb?action=PostNK`, {
        method: "POST",
        headers: {
