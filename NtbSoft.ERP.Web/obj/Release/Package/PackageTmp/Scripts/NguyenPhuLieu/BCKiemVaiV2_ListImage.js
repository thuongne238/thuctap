...
async function GetSoLoID() {
    ...
    const url = `/api/KiemVai/Get?action=GetBCSoLo&para1=${maNPL}&para2=${loc}&para8=1`;
    try {
...
async function GetMauVai() {
    ...
    const url = `/api/KiemVai/Get?action=GetBCMauVai&para1=${$("#SoLo").val()}&para2=${loc}&para8=1`;
    try {
        const response = await fetch(url);
        if (!response.ok) throw new Error(`Response status: ${response.status} `);
        let html = ``;
...
async function GetDot() {
    ...
    const maNPL = $("#MauVai").val();
    const url = `/api/KiemVai/Get?action=GetDotVai&para1=${soLoID}&para2=${maNPL}&para3=a&para8=1`;
...
async function GetBCKiemVai() {
    ...
    try {
        ...
        const maNPLselect = $("#MauVai").val();
        const soLoID = $("#SoLo").val();
        var dataUser = !window.CefSharp ? userNameSave : userName

        if ((dataUser.toUpperCase() == "KCS" || dataUser.toUpperCase() == "DIEMMY" || dataUser.toUpperCase() == "ADMIN") && !(await GetIsDaDuyetQC(soLoID, maNPLselect, dot, lanKiem))) {
            $('#btnShowModalXacNhanHuy').removeClass('d-none');
        } else {
            $('#btnShowModalXacNhanHuy').addClass('d-none');
        }
    } catch (error) {
        console.error(error.message);
    }
...
async function checkAllMinFabricByAPI(data, mauVTID) {
    ...
    const checks = data.map(async (item) => {
        const url = `/api/KiemVai/Get?action=CheckingSoCayBatchLot&para1=${soloVal}&para2=${mauVaiVal}&para3=${mauVTID}&para4=${item.MaNPL}&para8=1`;
...
async function GetLanKiem() {
    ...
    try {
        ...
        const url = `/api/KiemVai/Get?action=GetLanKiem&para1=${soLo}&para2=${maNPL}&para3=${dot}&para4=2&para8=1`;
        const response = await fetch(url);
        if (!response.ok) throw new Error(`Response status: ${response.status} `);
...
$(function () {
    ...
    $("#btnShowModalXacNhanHuy").on('click', async function () {
        const soLoID = $("#SoLo").val();
        const maNPL = $("#MauVai").val();
        const dot = $("#Dot").val();
        const lanKiem = $("#LanKiem").val();
        ...
        if (!maNPL) {
            showToast('warning', 'Vui lòng chọn thông tin cây vải')
            return;
        }
        const checkKiemVai = await GetIsCheckDuyetQC(soLoID, maNPL, dot, lanKiem)
        if (!checkKiemVai) {
            showToast('warning', `Item Code: ${itemCodeDisplay} chưa được xác nhận hoàn thành kiểm`)
            return;
        }
...
function showConfirmBatDauXaVai(onConfirm) {
    $('#btnXacNhanHuy').off('click');

    // Khi nhấn Đồng ý
    $('#btnXacNhanHuy').on('click', function () {
        if (typeof onConfirm === 'function') {
            onConfirm();
        }
        // Ẩn modal sau khi xử lý
        $("#confirmModalXacNhanHuy").modal('hide')

    });

    // Hiện modal
    $("#confirmModalXacNhanHuy").modal('show')
...
async function XacNhanHuy() {
    ...
    const url = `/api/KiemVai/Get?action=HuyXacNhan&para1=${maNPL}&para2=${soLOID}&para3=${dot}&para4=${lanKiem}&para5=${dataUser}&para8=1`;
    ...
    try {
        ...
        await response.json();
        showToast('success', 'Xác nhận hủy thành công')
    } catch (error) {
        showToast('success', 'Xác nhận hủy thất bại')
        console.error(error.message);
    }
}