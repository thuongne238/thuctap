...
function updateSummary(grid) {
    ...
    let str = parseFloat(total).toFixed(2);
    ...
    dxDataGridDangKyVatTu = $("#dxDataGridDangKyVatTu").dxDataGrid({
            ...
            },
            {
                ...
                headerCellTemplate: function (header, info) {
                    ...
                    const $icon = $("<i></i>")
                        ...
                        .on("click", function (e) {
                            ...
                            const dataSource = dxDataGridDangKyVatTu.option("dataSource");
                            dataSource.forEach(item => {
                                item.SLDK = Math.round((item.SLNhap ?? 0) * 100) / 100
                                refresh = true;
                            });
                ...
                cellTemplate: function (container, options) {
                    ...
                    const $icon = $("<i></i>")
                        ...
                        .on("click", function () {
                            options.data.SLDK = Math.round((value ?? 0) * 100) / 100;
                            dxDataGridDangKyVatTu.refresh();
                        });
                ...
                cellTemplate: function (container, options) {
                    ...
                    $input.on("input", function () {
                        ...
                        if (value.includes(".")) {
                            ...
                            this.value = intPart + "." + decPart.substring(0, 2); 
                            value = this.value;
                        }
                        ...
                        if (Number(value) > slConLai) {
                            showToast("warning", `SL Thu Hồi vượt quá SL còn lại được phép (${formatNumber(slConLai)}). CapPhat: ${formatNumber(capPhat)} - Tổng SLDK: ${formatNumber(tongSLDK)}`);
                            this.value = slConLai;
        ...
        `);
                    ...
                    $checkAll.on("change", function () {
                        ...
                        if (isChecked) {
                            // Chỉ tự chọn những dòng đã có SLDK > 0
...
function renderSignCell(container, options, fieldName) {
    ...
    if (data[fieldName] && data[fieldName] !== "checked") {
        ...
        const $wrapper = $("<div>").css({ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px" });
        $("<img>")
            .attr("src", `/Images/SignDeNghiThuHoi/${data[fieldName]}?${Date.now()}`)
            .css({ width: "70px", height: "35px" })
            .appendTo($wrapper);
        if (data[tenField]) {
            $("<span>").text(data[tenField])
                .css({ fontSize: "11px", color: "#333", fontWeight: "bold" })
                .appendTo($wrapper);
        }
        $wrapper.appendTo(containerDiv);
    ...
    // === Hàm kiểm tra quyền ký ===
    function checkPermission() {
        if (isSuperUser()) return true;

        if (fieldName === "SignNgDK") {
            // Người tạo phiếu mới được ký
        ...
        if (fieldName === "SignMer") {
            // Chỉ nhân viên phòng PKH (kế hoạch) mới được ký
    ...
    $("<input>").attr("type", "checkbox")
        ...
        .on("change", async function () {
            ...
            await UpdateKiTenPhieu(arrSave);
            _sendSignNotify(fieldName, rowData.PhieuTH);
        }).appendTo(containerDiv);
...
// Helper gửi notify sau khi ký
function _sendSignNotify(fieldName, phieu) {
    if (fieldName === "SignNgDK") {
                ...
                cellTemplate: function (container, options) {
                    ...
                    if (!isSuperUser()) {
                        if (signMer || signTBPMer) {
                            return; // chỉ SuperUser thấy
                        }
                        if (signTBP) {
                            if (user !== userTBP) return; // người khác ẩn hết

                            // TBP đó chỉ thấy nút xóa, KHÔNG thấy nút edit
                            const $wrapper = $(`<div></div>`).css({
                    ...
                    // Chưa ai ký hoặc SuperUser → hiện cả 2 nút
                    const $wrapper = $(`<div></div>`).css({
                        display: "flex", justifyContent: "center",
                        height: "25px", gap: "8px"
                    });
...
function getThuHoiRowKey(item) {
    return [
        item.MaLenhSanXuat, item.MaVT, item.MauVT, item.KhoVai, item.MaNPL
    ].map(v => String(v ?? "")).join("|");
}

function isThuHoiRowSelected(item) {
    const key = getThuHoiRowKey(item);
    return selectedRowsToSave.some(x => getThuHoiRowKey(x) === key);
}