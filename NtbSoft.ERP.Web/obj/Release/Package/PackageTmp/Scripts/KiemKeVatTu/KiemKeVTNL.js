...
let dxGridTongQuan;
let dxGridKiemKe;  // Bảng chưa kiểm kê
let dxGridKiemKeRight; // Bảng đã kiểm kê
...
let rowEditSLKK = {}
let rowComfirmtSLKK = {}
let lstRowComfirmtSLKK = []

$(function () {
    var dataUserCheck = !window.CefSharp ? userNameSave : userName
    if (dataUserCheck == null || !dataUserCheck) {
    ...
    $(".clearbarcode").on("click", function () {
        ...
        if ($matchedOption.length > 0) {
            ...
            $("#cayvai").val("all").select2('destroy').select2();

        }
        $("#khachhang").val("all").select2('destroy').select2();
        $("#mahang").val("all").select2('destroy').select2();
        $("#soloid").val("all").select2('destroy').select2();
        $("#tuNgay").val("all").select2('destroy').select2();
        $("#denNgay").val("all").select2('destroy').select2();
    })
    ...
    $("#cayvai").on("change", function () {
        //selectSoLo = null;
        // Reset flag khi chuyển vật tư
        isFirstLoadKiemKe = true;
        // reset map lại về rỗng
        loadPINCC();
    })
...
function clearInventoryTables() {
    ...
    $("#tblInventoryRight").empty();
    updateFooterStats('left');
    updateFooterStats('right');
}
async function performVTSwitch(selectNPL) {
    var $select = $("#cayvai");
    var $matchedOption = $select.find("option").filter(function () {
...
async function GetChiTietBarCode(barcode) {
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetDetailBarcode&para1=${barcode}`;
    try {
...
function CheckBarcodeTable(barcode) {
    var $tbody = $("#tblDataBody");
    var $targetRow = $tbody.find("tr").filter(function () {
        return $(this).find("td.barcode").text().trim() === barcode;
    });
    if ($targetRow.length > 0) {
        $tbody.prepend($targetRow); // đưa lên đầu
        $tbody.find("tr").removeClass("highlight");
        $targetRow.addClass("highlight"); // thêm màu để biết
    }
}
...
async function LoadChiTietVT() {
    ...
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetTQVT&para1=${maVT}&para2=${soloid}&para3=${maHang}&para4=${maKH}&para5=${tuNgayFormat}&para6=${denNgayFormat}`;
    ...
    try {
        ...
        let html = ``;
        data.map(item => {
            var trangthai = item.Status == 0 ? "Nhập kho" : item.Status == 1 ? "Xuất hàng" : item.Status == 2 ? "Thu hồi" : "Chưa nhập kho"
            var className = item.Status == 0 ? "green" : item.Status == 1 ? "red" : item.Status == 2 ? "#8d54f7" : "#D39E00"
            html += `
                <tr data-slnhapkho="${item.SLNK || ""}" data-ngaynk="${item.Status != 4 ? item.NgayNK ?? "" : ""}" data-status="${item.Status}" data-barcode="${item.BarCode}">
                        ...
                        <td class="">${item.Status != 4 ? item.NgayNK : ""}</td>
                        <td style="color:${className};min-width:40px" class="">${parseFloat(item.SLNK.toFixed(2)) || ""}</td>
                        <td class="">${item.NgayXH}</td>
                        <td style="color:${className};min-width:40px" class="">${parseFloat(item.SLXH.toFixed(2)) || ''}</td>
                        <td class="">${item.Status == 2 ? item.NgayTK : ""}</td>
                        <td style="color:${className};min-width:40px" class="">${item.Status == 2 ? parseFloat(item.SLTK.toFixed(2)) || '' : ""}</td>
                        <td class="dongia">${item.DonGia}</td>
                         <td class="thanhtien">${item.ThanhTien}</td>
                        <td class="">
                           <div>
                          <i style="cursor: pointer;" class="fa-solid fa-server"></i>
                            </div>
                        </td>
        ...
        let slnk = data.reduce((acc, cur) => acc + parseFloat(cur.SLNK), 0);
        let slxh = data.reduce((acc, cur) => acc + parseFloat(cur.SLXH), 0);

        var sltk = parseFloat(slnk) - parseFloat(slxh)
        let htmltfoot = `
                        <tr >
                        <td style="text-align: left; padding-left: 20px;" colspan="20" class="">
                            <div style="display: flex; gap: 10px; font-size: 14px; font-weight: bold; color: green;">
                                <span>Tồn kho: ${parseFloat(sltk.toFixed(2)).toLocaleString()} </span> --
                                <span>Xuất hàng: ${parseFloat(slxh.toFixed(2)).toLocaleString()} </span>
                             </div>
                        </td>
            ...
            `
        $("#tblDataBody").html(html)
        $("#tfoot").html(htmltfoot)
        CheckBarcodeTable(barcodeLocal)
    } catch (error) {
        console.error(error.message);
    }
}
...
function startScan() {
    ...
    html5QrCode.start(
        ...
        (decodedText, decodedResult) => {
            ...
            lastScanTime = now;
            barcodeLocal = decodedText
            if ($("#cayvai").val() == "all") {
                GetVTBarCode(decodedText)
                $("#QRScan").val("")
            } else {
                CheckBarcodeTable(decodedText)
            }
            $("#QRScan").val("")
        },
        (errorMessage) => {
...
async function LoadChiTietVTKiemKe(value) {
    ...
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetPhieuKiemKe&para1=${maVT}&para2=${soloid}&para3=${maHang}&para4=${maKH}&para5=${phieuVTKK}&para6=${denNgayFormat}&para7=1`;
...
function updateFooterStats(side) {
    ...
    const footerHtml = `
        <tr style="background-color: #f8f9fa; font-weight: bold;">
            ...
            <td class="text-center">Số lượng : ${totalQuantity.toFixed(2).toLocaleString()}</td>
...
async function GetVTBarCode(barcode) {
    ...
    try {
        ...
        //// TRƯỜNG HỢP 1: VẬT TƯ ĐÚNG
            ...
            if ($("#ipTrangThai").val() == "Đã Xác Nhận") {
                showToast("warning", `ItemCode: ${maVTInput} đã được duyệt.Không được kiểm kê.!`)
                return;
            }
...
function getItemInfoFromTongQuan(barcode) {
    if (!dxGridTongQuan || !barcode) {
        console.warn("Không thể lấy thông tin từ Tổng Quan");
        return null;
    }

    const allData = dxGridTongQuan.getDataSource().items();
...
function updateThongTinSauKhiScan(barcode) {
    ...
    if (!targetItem && dxGridKiemKe) {
        const leftData = dxGridKiemKe.getDataSource().items();
        targetItem = leftData.find(item => item.BarCode === barcode);
    }

    if (!targetItem && dxGridKiemKeRight) {
        const rightData = dxGridKiemKeRight.getDataSource().items();
...
async function handleCorrectVT(activeTab, barcode) {
    if (activeTab === 'inventory-tab') {
        const leftData = dxGridKiemKe ? dxGridKiemKe.getDataSource().items() : [];
...
$("#soloid").on("select2:select", function () {
    ...
    if (value === "all") {
        ...
        dxGridTongQuan.clearFilter();
        dxGridKiemKe.clearFilter();
    } else {
        // Apply filter cho SoLoID cụ thể
        dxGridTongQuan.filter(["SoLoID", "=", value]);
        dxGridKiemKe.filter(["SoLoID", "=", value]);
    }

    dxGridTongQuan.refresh();
});
// Hàm tạo dxGrid cho Tổng quan
function createViewDxDataTongQuan() {
    dxGridTongQuan = $("#dxDataTongQuan").dxDataGrid({
        dataSource: [],
...
async function LoadChiTietVTDx() {
    ...
    try {
        ...
        if (!dxGridTongQuan) return;

        dxGridTongQuan.beginUpdate();
        dxGridTongQuan.clearFilter();
        dxGridTongQuan.option({
            dataSource: data,
        });
        dxGridTongQuan.endUpdate();
    } catch (error) {
        console.error(error.message);
    }
}
...
function CheckBarcodeTableDx(barcode) {
    ...
    const dataSource = dxGridTongQuan.getDataSource();
    ...
    if (targetIndex !== -1) {
        ...
        dxGridTongQuan.beginUpdate();
        dxGridTongQuan.option('dataSource', allData);
        dxGridTongQuan.endUpdate();

        // Scroll to top
        dxGridTongQuan.getScrollable().scrollTo(0);

        // Highlight row
        setTimeout(() => {
            const rowElement = dxGridTongQuan.getRowElement(0);
...
// Gọi hàm khi modal mở hoặc sau khi quét barcode thành công
/// ======== TAB KIỂM KÊ ======
async function LoadChiTietVTKiemKeDx(value) {
    ...
    try {
        ...
        if (!dxGridKiemKe) return;

        // Load dữ liệu vào bảng trái
        dxGridKiemKe.beginUpdate();
        dxGridKiemKe.clearFilter();
        dxGridKiemKe.option({
            dataSource: data,
            repaintChangesOnly: true
        });
        dxGridKiemKe.endUpdate();

        // Xóa dữ liệu bảng phải nếu load mới
        if (maVT && maVT !== "" && maVT !== "all") {
            await reloadKiemKeRightByMaVT(maVT);
        } else if (!value) {
            dxGridKiemKeRight.beginUpdate();
            dxGridKiemKeRight.option('dataSource', []);
            dxGridKiemKeRight.endUpdate();
        }
...
function createViewDxDataKiemKe() {
    dxGridKiemKe = $("#dxDataKiemKe").dxDataGrid({
        dataSource: [],
        columnAutoWidth: false,
        allowColumnResizing: false,
        columnHidingEnabled: false,
        ...
        columns: [
            {
                dataField: "SoLo", caption: "PI NCC", alignment: "center", width: 160,
                cellTemplate: function (container, options) {
                    ...
                    const $icon = $("<i></i>")
                        .addClass("fa-solid fa-circle-right")
                        .css({ position: "absolute", right: 1, color: 'green', fontSize: 16, cursor: "pointer" })
                        .on("click", function (e) {
                            ...
                            $('#itemCode').text(item.MaVT);
                            $('#color').text(item.MauVT);
                            $('#khosizeM').text(item.KhoVai);
                            $('#roll').text(item.SoKien);
                            $('#quantity').text(item.SLNK);
                            ...
                            showConfirmModalXacNhan(function () {
                                const ghiChu = $('#note').val().trim();

                                if (!ghiChu || ghiChu == '') {
                                    showToast('warning', 'Vui lòng nhập ghi chú để chuyển nhanh');
                                    return;
                                }

                                handleInventoryScanDx(item.BarCode, true, ghiChu,1)
                                $('#inventoryModal').modal('hide')
                                $('#inventoryQty').prop('disabled', false);
                            })
                        })
            ...
            calculateCustomSummary: function (options) {
                if (options.name === "tongKho") {
                    ...
                    if (options.summaryProcess === "finalize") {
                        ...
                        options.totalValue = `${totalRoll.toLocaleString()} Roll/Kiện -- Số lượng: ${totalNH.toLocaleString()}`;
                    }
                }
            }
        },
    ...
    // Bảng đã kiểm kê
    dxGridKiemKeRight = $("#dxDataKiemKeRight").dxDataGrid({
        dataSource: [],
        keyExpr: "BarCode",
        ...
        columns: [
            ...
            },
            {
                ...
                cellTemplate: function (container, options) {
                    const rowData = options.data;
                    const slnk = parseFloat(rowData.SLNK.toFixed(2)) || 0;
                    const slkk = parseFloat(rowData.SLKiemKe.toFixed(2)) || 0;
            ...
            },
            {
                ...
                cellTemplate: function (container, options) {
                    ...
                    const currentValue = parseFloat(rowData.SLKiemKe.toFixed(2)) || 0;
                    let resultValueDisplay = formatNumber(currentValue)
                    const slnk = parseFloat(rowData.SLNK.toFixed(2)) || 0;
                    const CheckSave = rowData.CheckSave
                    ...
                    const $editIcon = $(`<i class="${CheckSave == 0 ? "" : "d-none"}">` )
            ...
            { dataField: "TenDVVT", caption: "Tên DVVT", alignment: "center", width: 90 },
            {
                dataField: "GhiChuKK",
                caption: "Ghi chú",
                width: 250,
            ...
            },
            {
                ...
                cellTemplate: function (container, options) {
                    ...
                    if (rowData.IsXacNhan === 1) return; // ẩn nếu đã xác nhận

                    const $btn = $('<button>')
                        .addClass('btn btn-danger btn-sm')
                        .html('<i class="fa fa-trash"></i>')
                        .attr('title', 'Xóa cây vải')
                        .click(function () {
                            showDeleteModal(rowData, async function () {
                                const $deleteBtn = $("#deleteConfirmBtn");

                                // Disable ngay khi bấm
                                $deleteBtn.prop("disabled", true).html('<i class="fa fa-spinner fa-spin"></i> Đang xóa...');
                                ...
                                const success = await DeleteCayVaiKiemKe(arrSave);

                                if (!success) {
                                    // Enable lại nếu thất bại
                                    $deleteBtn.prop("disabled", false).html('<i class="fa fa-trash me-1"></i>Xóa');
                                    showToast("error", "Xóa thất bại, vui lòng thử lại!");
                                    return false;
                                }

                                const rowIndex = dxGridKiemKeRight.getRowIndexByKey(rowData.BarCode);
                                dxGridKiemKeRight.deleteRow(rowIndex);
                                LoadChiTietVTKiemKeDx();
        ...
        summary: {
            ...
            calculateCustomSummary: function (options) {
                if (options.name === "tongKho") {
                    ...
                    if (options.summaryProcess === "finalize") {
                        const { totalRoll } = options.totalValue;
                        options.totalValue = `${totalRoll} Roll/Kiện`;
                    }
                }

                if (options.name === "slkkedit") {
                    if (options.summaryProcess === "start") {
                        options.totalValue = 0;
                    }
                    if (options.summaryProcess === "calculate") {
                        const slEdit = options.value.SLKiemKeEdit;
                        const slKiemKe = options.value.SLKiemKe;

                        const valueToSum = (slEdit !== null && slEdit !== undefined && slEdit !== 0)
                            ? slEdit
                            : slKiemKe;

                        options.totalValue += valueToSum || 0;
                    }
                    if (options.summaryProcess === "finalize") {
...
async function DeleteCayVaiKiemKe(arrSave) {
    const url = `/api/PhieuXuatHangNPL/PostChiTietKiemKeNPL?action=DeleteCayVaiKiemKe`;
    try {
...
function showDeleteModal(rowData, onConfirm) {
    // Điền thông tin vào modal
    $("#del-itemCode").text(rowData.MaVT || "—");
    $("#del-color").text(rowData.MauVT || "—");
    $("#del-size").text(rowData.KhoVai || "—");
    $("#del-roll").text(rowData.SoKien || "—");
    $("#del-barcode").text(rowData.BarCode || "—");

    // Gắn sự kiện nút Xóa (off trước để tránh duplicate)
    $("#deleteConfirmBtn").off("click").on("click", async function () {
        if (typeof onConfirm === "function") {
            const result = await onConfirm();
            if (result === false) return; // ❌ không đóng nếu lỗi
        }
        $("#deleteConfirmModal").modal("hide"); // ✅ đóng khi thành công
    });
...
// ======== Kiểm tra handleInventoryScanDx có gọi đúng không ========
async function handleInventoryScanDx(barcode, isChuyenNhanh, ghiChuChuyenNhanh,checkSave = 0) {
    ...
    const leftDataSource = dxGridKiemKe.getDataSource();
    ...
    if (targetIndex !== -1) {
        ...
        fullInfo.UserKK = dataUser;
        fullInfo.IsNPL = 1;
        fullInfo.SLKiemKe = fullInfo.SLNK;
        fullInfo.PhieuKiemKe = $("#phieuvattukiem").val()
        if (isChuyenNhanh) {
            fullInfo.SLKiemKeEdit = 0;
            fullInfo.GhiChu = ghiChuChuyenNhanh;
        }
        fullInfo.MauVTID = checkSave
        ...
        try {
            ...
            if (result === "True" || response.ok) {
                ...
                dxGridKiemKe.beginUpdate();
                dxGridKiemKe.option('dataSource', leftData);
                dxGridKiemKe.endUpdate();
                highlightBarcodeInRightGrid(barcode);
                // Scroll to top bảng phải
                dxGridKiemKeRight.getScrollable().scrollTo(0);
            }
        } catch (err) {
        ...
        // Xử lý chuyển data
        //const rightDataSource = dxGridKiemKeRight.getDataSource();
        ...
        //rightData.unshift(targetItem);
        //dxGridKiemKeRight.beginUpdate();
        //dxGridKiemKeRight.option('dataSource', rightData);
        //dxGridKiemKeRight.endUpdate();

        //dxGridKiemKeRight.getScrollable().scrollTo(0);

        //setTimeout(() => {
        //    const rowElement = dxGridKiemKeRight.getRowElement(0);
        //    if (rowElement && rowElement.length > 0) {
        ...
        if (existInRight !== -1) {
            ...
            setTimeout(() => {
                const rowElement = dxGridKiemKeRight.getRowElement(existInRight);
                if (rowElement && rowElement.length > 0) {
                    $('.dx-row').removeClass('highlight');
                    $(rowElement).addClass('highlight');
                    dxGridKiemKeRight.getScrollable().scrollToElement($(rowElement));
                }
            }, 100);
        } else {
            if (!isPostSwitchScan) {
...
function clearInventoryTablesDx() {
    if (dxGridKiemKe) {
        dxGridKiemKe.beginUpdate();
        dxGridKiemKe.option('dataSource', []);
        dxGridKiemKe.endUpdate();
    }

    if (dxGridKiemKeRight) {
        dxGridKiemKeRight.beginUpdate();
        dxGridKiemKeRight.option('dataSource', []);
        dxGridKiemKeRight.endUpdate();
    }
...
}// ======== HÀM CẬP NHẬT GRID LỊCH SỬ QUÉT ========
...
async function GetVatTuKiemKe() {
    ...
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetVTMK&para1=1&para2=${makh}&para3=${mahang}&para4=${tuNgayFormat}&para5=${denNgayFormat}`;
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Response status: ${response.status}`);
...
async function reloadKiemKeRightByMaVT(maVT) {
    ...
    const denNgayFormat = $("#filterType").val() != "khachhang"
        ...
        : "1990-01-01";
    const phieuVTKK = $("#phieuvattukiem").val()
    // API để lấy dữ liệu đã kiểm kê (status=1 hoặc tùy logic của bạn)
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetKiemKe&para1=${maNPL}&para2=${soloid}&para3=${maHang}&para4=${maKH}&para5=${phieuVTKK}&para6=${denNgayFormat}&para7=1`;
    ...
    try {
        ...
        const data = await response.json();
        if (!dxGridKiemKeRight) return;

        // Cập nhật dữ liệu bảng phải
        dxGridKiemKeRight.beginUpdate();
        dxGridKiemKeRight.option({
            dataSource: data,
            repaintChangesOnly: true
        });
        dxGridKiemKeRight.endUpdate();
...
function highlightBarcodeInRightGrid(barcode) {
    if (!dxGridKiemKeRight || !barcode) return;
    ...
    setTimeout(() => {
        ...
        if (targetIndex !== -1) {
            ...
            // Highlight row mới
            setTimeout(() => {
                const rowElement = dxGridKiemKeRight.getRowElement(targetIndex);
                if (rowElement && rowElement.length > 0) {
                    $(rowElement).addClass('highlight');
                }
            }, 100);
...
async function loadDataChiTietKiemKe(maNPL) {
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetChiTietKiemKe&para1=${maNPL}`;
    try {
        const response = await fetch(url)
...
async function GetPhieuVatTuKK() {
    const url = `/api/PhieuXuatHangNPL/GetDS?action=GetPhieuVatTuKK&para1=1`;
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Response status: ${response.status}`);
        ...
        data.Table.forEach(x => {
            html += `<option data-HoanThanh= "${x.HoanThanh}" value="${x.PhieuVatTuKK}">Phiếu VTKK: ${x.Dot} - ${x.NgayTao} - ${x.HoanThanh}</option>`
...
async function GetVatTuKK(value) {
    const phieuVTKK = $("#phieuvattukiem").val()
    const url = `/api/PhieuXuatHangNPL/GetDS?action=GetVatTuKiemKe&para1=1&para2=${phieuVTKK}`;
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Response status: ${response.status}`);
...
$(document).ready(function () {
    ...
    $(document).on('click', '#tableBody tr', function () {
        ...
        const trangThai = selectedItem.IsLog == 0 ? "Đã Xác Nhận" : "Chưa Xác Nhận"
        $("#ipTrangThai").css({
            "font-size": "14px",
            "color": $("#phieuvattukiem option:selected").data("hoanthanh") == 1 ? `green` : `red`
        }).val(trangThai);
        if (selectedItem) {
...
function renderSwitch(manpl) {
    const selectedItem = allData.find(item => item.MaNPL.toUpperCase() === manpl.toUpperCase());
...
async function Export() {
    let dataEX = dxGridKiemKeRight.option("dataSource")
    const displaySoPhieu = $('#phieuvattukiem').val();
    const displayMaVT = $('#displayInput').val();
    const phieuVatTuKK = $("#phieuvattukiem").val();
    const itemCode = $("#displayInput").val();

    if (!phieuVatTuKK || phieuVatTuKK == "") {
        showToast("warning", "Vui lòng chọn phiếu !")
    }
    if (!itemCode || itemCode == "") {
        showToast("warning", "Vui lòng chọn item code !")
    }
    ...
    const fileName = `DSKiemKeNL_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}_${displaySoPhieu}_${displayMaVT}.xlsx`;
...
$(function () {
    $("#confirmVT").on("click", function () {
        ...
        var grid = $("#dxDataKiemKe").dxDataGrid("instance");
        var dataSource = grid.option("dataSource");
        if (dataSource.length > 0) {
            showToast("warning", `ItemCode: ${maVTInput} chưa được kiểm kê hết Roll/Kiện.!`)
            return;
        }

        openConfirmModal(2)
        showConfirmModalXacNhanPhieu(async function () {
            const module = 'M.10.16.00'
            const title = ' '
    ...
    $("#confirmPhieu").on("click", async function () {
        const $btn = $(this);
        ...
        try {
            ...
            const isValid = await KiemTraVatTuChuaKiemKe();
            ...
            showConfirmModalXacNhanPhieu(async function () {
                ...
                await sendNotify(module, title, detail, sendTo, BoPhan, Status);
...
async function KiemTraVatTuChuaKiemKe() {
    const danhSachChuaKiem = [];
    const rows = $("#tableBody tr").toArray();

    if (rows.length === 0) {
        showToast("warning", "Không có danh sách vật tư để kiểm tra !");
        return false;
    }

    for (const row of rows) {
        const maNPL = $(row).attr("data-manpl");
        const tenNPL = $(row).find("td:first").text().trim();

        if (!maNPL) {
            continue;
        }

        const data = await CheckVatTuKiemKe(maNPL);

        if (!data) {
            showToast("error", `Không kiểm tra được ItemCode ${tenNPL}. Vui lòng thử lại !`);
            return false;
        }

        if (data.length > 0) {
            danhSachChuaKiem.push(tenNPL);
        }
    }

    if (danhSachChuaKiem.length > 0) {
        showToast(
            "warning",
            `Các ItemCode chưa được kiểm kê hết Roll/Kiện:<br>${danhSachChuaKiem.join("<br>")}`
        );
        return false;
    }
...
async function CheckVatTuKiemKe(manpl) {
    var maVT = manpl;
    var soloid = $("#soloid").val();
    var maKH = $("#filterType").val() != "khachhang" ? "all" : $("#khachhang").val();
    var maHang = $("#filterType").val() != "khachhang" ? "all" : $("#mahang").val();
    ...
    const url = `/api/PhieuXuatHangNPL/GetTH?action=GetPhieuKiemKe&para1=${maVT}&para2=${soloid}&para3=${maHang}&para4=${maKH}&para5=${phieuVTKK}&para6=""&para7=1`;
...
function showConfirmModalXacNhanPhieu(onConfirm) {
...
function OpenPhieu() {
    const textPhieu = $("#phieuvattukiem option:selected").text();
...
$(function () {
    ...
    const picker1 = new tempusDominus.TempusDominus(document.getElementById('datetimepicker3'), {
        display: {
            components: {
    ...
    const picker2 = new tempusDominus.TempusDominus(document.getElementById('datetimepicker4'), {
        display: {
            components: {
                calendar: true, date: true, month: true, year: true,
                clock: false, hours: false, minutes: false, seconds: false
    ...
    picker2.dates.setValue(new tempusDominus.DateTime(endDayOfMonth));
...
let dxXuatPhieu;
function createViewDxXuatPhieu(data) {
    dxXuatPhieu = $("#dxXuatPhieu").dxDataGrid({
        dataSource: data,
        noDataText: "Chưa có dữ liệu",
        columnAutoWidth: false,
        allowColumnResizing: false,
...
async function GetThongTinXuatPhieu() {
    try {
        ...
        const url = `/api/PhieuXuatHangNPL/GetTH?action=GetXuatPhieu&para1=${tuNgay}&para2=${denNgay}&para3=1`;


        const response = await fetch(url);
        const data = await response.json();

        createViewDxXuatPhieu(data);
...
async function Export3() {
    try {
        ...
        const dataSource = dxXuatPhieu.option("dataSource").filter(item => item.isCheck == true);
        if (dataSource.length == 0) {
        ...
        const tuNgay = moment($('#tuNgayXP').val(), 'DD/MM/YYYY').format('YYYY-MM-DD')
        const denNgay = moment($('#denNgayXP').val(), 'DD/MM/YYYY').format('YYYY-MM-DD')
        const url = `/api/phieuxuathangnpl/ExportFileXuatPhieuGhiNhanKiemKeNL?action=gettongquankiemkev2&para1=${dataPhieuXuat}&para2=${tuNgay}&para3=${denNgay}&para4=1`;
...
function ExportPhieu() {
    const maPhieu = $("#phieuvattukiem").val();
    if (!maPhieu) {
    ...
    const tungayformat = "1990-01-01";
    const denngayformat = "1990-01-01";
    const url = `/api/phieuxuathangnpl/exportfileghinhankiemkenl?action=gettongquankiemkev2&para1=${maPhieu}&para2=${tungayformat}&para3=${denngayformat}&para4=1`;
