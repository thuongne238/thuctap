...
function duLieuPhieu() {
    ...
    let object = {
        ...
        XeRomoc: xeRoMoc,
        SoContCu: module == 2 ? valMaContModule2 : valMaCont,
        MaPKL_XH: valPKL,
...
function ThongBao() {
    ...
    if ($(".banSo").val().trim() == "") {
        ...
        $(".banSo").focus();
    } else if ($(".container").val().trim() == "") {
        errorMessage += 'Container bị rỗng ';
        $(".container").focus(); // 
...
$(".finishDongHang").on("change", function () {
    ...
    let ngayktdh = new Date(ngaythang);
    let ngaynhapkho = new Date(valNgayNhapKho);

    var parts1 = formatDateTimeAdd(ngayktdh).split('/');
    var parts2 = formatDateTimeAdd(ngaynhapkho).split('/');

    var d1 = new Date(parts1[2], parts1[1] - 1, parts1[0]);
    var d2 = new Date(parts2[2], parts2[1] - 1, parts2[0]);
    if (d1 < d2) {
        Swal.fire({
            title: `Ngày kt đóng hàng ${formatDateTimeAdd(ngayktdh)} không được nhỏ hơn ngày nhập kho ${formatDateTimeAdd(ngaynhapkho)}`,
            icon: 'warning',
...
function renderMaPhieu() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            valMaContModule2 = data[0].MaCont
            if (!window.CefSharp)
            ...
            $.each(data, function (index, item) {
                ...
                $(".lookup-results").append(`
                    <li class="result-row" data-cont="${item.SoCont}" data-ngaynhapkho="${item.NgayNhapKho}"  data-masocont="${item.MaCont}" data-value=${item.MaPhieuBB} data-name="${item.TenBienBan}">
        ...
        success: function (data) {
            ...
            $.each(data, function (index, item) {
                $(".lookup-results1").append(`  
                    <li class="result-row" data-ngaynhapkho="${item.NgayNhapKho}" data-isexport="${item.IsExport}" data-seal="${item.MaSeal}" data-macont="${item.MaCont}" data-cont="${item.Cont}"
...
function renderTableMaHang() {
    ...
    AutoWidth()
    $.ajax({
        url: "/api/XuatHang/Get?Action=GetTableTenHang&para1=" + valPKL + "&para2=" + valMaCont,
        type: 'Get',
        success: function (data) {
            $("#bodyMahang").empty();
            ...
            $.each(data, function (index, item) {
                $("#bodyMahang").append(`
                <tr data-poid="${item.POID}" data-mahang=${item.MaH} style="height:30px">
                            <td>${item.TenHang}</td>
                            <td>${item.MaHang}</td>
                            <td>${item.PO}</td>
                            <td>${item.SoLuong}</td>
                            <td>${item.SLThung}</td>
                            <td style="outline: none; text-align: center;" contenteditable='true' >${item.GhiChu}</td>
                        </tr>
...
function renderTableTenHang() {
    $("#bodyMahang").empty();
    $.ajax({
        url: "/api/XuatHang/Get?Action=GetTenHang&para1=" + maPhieu + "&para2=" + valMaContModule2,
        type: 'Get',
        success: function (data) {
            let totalSoLuong = data.reduce((acc, curr) => acc + parseInt(curr.SoLuong), 0);
            let totalSoKien = data.reduce((acc, curr) => acc + parseInt(curr.SLThung), 0);
...
function Export() {
    var url = "/api/XuatHang/BCExcelBB?Action=GetBCEXXuathang&para1=" + maPhieu + "&para2=A" + "&para3=" + valMaContModule2;
...
$(document).on("click", function (event) {
    if (!$(event.target).closest('.maSeal, .inputName,.input_image').length) {
...
function saveGhiChu() {
    let arrGhiChu = []
    $("#bodyMahang tr").each(function () {
        let row = $(this).find('td');
        let poid = $(this).data("poid")
        let mahang = $(this).data('mahang')
        if (row.eq(5).text().trim() == "") return true;
        let object = {
            MaPKL_XH: valPKL,
            VCCont: valMaCont,
            MaHang: mahang,
            PO: poid,
            GhiChu: row.eq(5).text()
        };
...
function ChiTietXuatHang() {
    $.ajax({
        url: "/api/ThongKeDongThung/GetCTXH?&para1=" + valPKL + "&para2=A" + "&Para3=a",
        type: 'Get',
        success: function (data) {
            $(".modal-title").text("Mã PKL :" + valPKL + " - Cont: " + valCont)
            if (data.length == 0) return;
            data = data.filter(item => item.Cont === valMaCont)
...
function saveListSeal(maPhieu, status) {
    ...
    $(".maSealSelect option:selected").each(function () {
        ...
        let objcet = {
            MaPhieu: maPhieu,
            MaCont: module == 2 ? valMaContModule2 : valMaCont,
            MaSeal: item,
            TenSeal: text,
            MaPKL: valPKL,
            SoXe: $(".banSo").val().toUpperCase(),
...
function getDsXe() {
    let arrDsSeal = []
    $.ajax({
        url: `/api/BienBanLuuSeal/Get?Action=GetDSXe&para1=${valMaContModule2}&para2=${valPKL}&para3=a&para4=a&para5=a`,
        type: "GET",
        success: function (data) {
            let html = '';
...
function deleteDSXe() {
    $.ajax({
        url: `/api/BienBanLuuSeal/Get?Action=DeleteDSXe&para1=${valMaContModule2}&para2=${valPKL}&para3=a&para4=a&para5=a`,
        type: "GET",
        success: function (data) {
