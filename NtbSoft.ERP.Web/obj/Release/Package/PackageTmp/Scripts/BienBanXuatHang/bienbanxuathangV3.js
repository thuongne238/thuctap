...
function getMaxPhieu(arrDataPost) {
    $.ajax({
        url: `/api/BienBanLuuSeal/Get?Action=GetMaPhieu&para1=${maPhieu}&para2=a&para3=a&para4=a&para5=a`,
        type: "Get",
        success: function (data) {
            let maphieubb = 0;
            if (data === null || data.length == 0) {
                ...
                maphieubb = Number(data[0].MaPhieuBB) + 1
            }
            ...
            arrDataPost.map(item => {
                let object = {
                    ...
                    CMND: $(".CMND").val(),
                    CongTy: $(".congty").val(),
                    NgayXeDen: moment($(".xeDen").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    NgayGioCheckXeRoMoc: moment($(".checkXe").val(), "DD/MM/YYYY").format("YYYY-MM-DD"),
                    NgayGioDongHang: $(".donghang").val() == "" ? "1990-01-01" : moment($(".donghang").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    NgayGioKTDongHang: $(".finishDongHang").val() == "" ? "1990-01-01" : moment($(".finishDongHang").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    MaSeal: xeVC == 2 ? selectedString : $(".maSeal").val(),
                    NgayGioXeDi: $(".xeDI").val() == "" ? "1990-01-01" : moment($(".xeDI").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    XeRomoc: xeRoMoc,
                    ...
                    ChungTu: $(".chungtu").val().toUpperCase(),
                    TGBVCheckBD: moment($(".bvcheckbd").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    TGBVCheckKT: moment($(".bvcheckkt").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    TGNVCheckBD: moment($(".nvcheckbd").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                    TGNVCheckKT: moment($(".nvcheckkt").val(), "DD/MM/YYYY HH:mm").format("YYYY-MM-DD HH:mm"),
                }
...
async function renderDataPost(arrDataPost) {
    getMaxPhieu(arrDataPost)
...
function postAPIDLPhieu(dataPhieu, maphieubb, arrDataPost) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            luuNoiDung(maphieubb);
            saveImageOther(maphieubb)
            saveKyTen(maphieubb)
            saveGhiChu();
            if (module == 2)
                return;
...
function getTenBienBanEndSave() {
    $.ajax({
        ...
        success: function (data) {
            let tenBienBanEnd = ""
            if (data.length != 0)
                tenBienBanEnd = data[0].TenBienBan
...
$("#Update").on("click", function () {
    ...
    if (module == 2) {
        if ($(".container").val().trim() == "") {
            showToast('warning', "Không có phiếu")
        } else {
            saveAll()
        }
...
function ThongBao() {
    var errorMessage = '';
    if ($(".banSo").val().trim() == "") {
        errorMessage += 'Bản số xe bị rỗng ';
        $(".banSo").focus();
        ...
        errorMessage += 'Ngày ETD bị rỗng ';
        $(".checkXe").focus();
    }
    else if ($(".donghang").val().trim() == "") {
        errorMessage += 'TG bắt đầu đóng cont ';
        $(".donghang").focus();
    }
    else if ($(".finishDongHang").val().trim() == "") {
        errorMessage += 'TG kết thúc đóng cont ';
    ...
    else if ($(".bvcheckbd").val().trim() == "") {
        errorMessage += 'Thời gian BV kiểm tra từ bị rỗng';
        $(".bvcheckbd").focus();
    }
    else if ($(".bvcheckkt").val().trim() == "") {
        errorMessage += 'Thời gian BV kiểm tra đến bị rỗng';
        $(".bvcheckkt").focus();
    }
    else if ($(".nvcheckbd").val().trim() == "") {
        errorMessage += 'Thời gian NV kho kiểm tra từ bị rỗng';
        $(".nvcheckbd").focus();
    }
    else if ($(".nvcheckkt").val().trim() == "") {
        errorMessage += 'Thời gian NV kho kiểm tra đến bị rỗng';
        $(".nvcheckkt").focus();
    }
    showToast('warning', 'Vui lòng nhập đầy đủ thông tin cho các ô sau: ' + errorMessage);
    $("#Update").prop("disabled", false);

}
let arrNoiDung = [];

function luuNoiDung(maphieu) {
    let maphieubb = maphieu;
    if (module == 2) maphieubb = maPhieu
    let index = 1;
    arrNoiDung = []
    $('#tbody tr').slice(0, -1).each(function (i) {
        ...
        var secondColumnText = $(this).find('td').eq(1).text();
        let object = {
            MaBBPhieu: maphieubb,
            MaNoiDung: index,
            NoiDung: secondColumnText,
...
function handleDateChange(selector, outputSelector) {
    $(selector).on("change", function () {
        ...
        if (selector == ".bvcheckbd") {
            $(".bvcheckbd").val(datetime)
            $(".bvcheckkt").val(datetime)
            $("#datetime_bvcheckbd").text(formatDateTime(datetime))
            $("#datetime_bvcheckkt").text(formatDateTime(datetime))
        }
        else if (selector == ".nvcheckbd") {
            $(".nvcheckbd").val(datetime)
            $(".nvcheckkt").val(datetime)
            $("#datetime_nvcheckbd").text(formatDateTime(datetime))
            $("#datetime_nvcheckkt").text(formatDateTime(datetime))
        }
        else if (selector == ".checkXe") {
            $(outputSelector).text(formatDateTimeAdd(datetime));
        } else {
...
handleDateChange(".bvcheckbd", "#datetime_bvcheckbd");
handleDateChange(".bvcheckkt", "#datetime_bvcheckkt");
handleDateChange(".nvcheckbd", "#datetime_nvcheckbd");
handleDateChange(".nvcheckkt", "#datetime_nvcheckkt");

$(".finishDongHang").on("change", function () {
    let ngaythang = $(this).val()
    if (module == 1) {
        if (arrMaPKL_Cont.length == 0) {
            showToast('warning', "Vui lòng chọn pkl")
            return;
        }
        ...
        $.ajax({
            url: `/api/BienBanLuuSeal/Get?Action=GetNgayNhapKho&para1=${maPhieu}&para2=a&para3=a&para4=a&para5=a`,
            type: "Get",
            success: function (data) {
                data.map(item => {
                    let ngaythangngayNK = item.NgayNhapKho
...
function formatDateTimeAddA(datetime) {
    const date = new Date(datetime);
    const day = String(date.getDate()).padStart(2, '0');
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const year = date.getFullYear();
...
function setDateTimePicker(pickerId, datetimeStr) {
    if (!datetimeStr) return;
    const el = document.getElementById(pickerId);
    if (!el) return;

    // Thử các cách lấy instance khác nhau tùy version
    const instance = el._tempusDominus
        || el.tdInstances
        || tempusDominus.TempusDominus.instances?.get(el)
        || $(el).data('tempusDominus');

    if (instance) {
        try {
            instance.dates.setValue(tempusDominus.DateTime.convert(new Date(datetimeStr)));
        } catch (e) {
            // Fallback: set thẳng vào input
            const input = el.querySelector('input') || el;
            input.value = formatDateTime(datetimeStr);
        }
    } else {
...
function setDatePicker(pickerId, datetimeStr) {
    if (!datetimeStr) return;
    const el = document.getElementById(pickerId);
    if (!el) return;
    ...
    if (instance) {
        try {
            ...
            const input = el.querySelector('input') || el;
            input.value = formatDateTimeAdd(datetimeStr);
        }
    } else {
        const input = el.querySelector('input') || el;
        input.value = formatDateTimeAdd(datetimeStr);
...
function renValueNoiDung() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // ---- Các field ngày dùng tempus-dominus ----
            setDateTimePicker('dp_xeDen', d.NgayXeDen)
            setDatePicker('dp_checkXe', d.NgayGioCheckXeRoMoc)
            setDateTimePicker('dp_bvCheckBD', d.TGBVCheckBD)
            setDateTimePicker('dp_bvCheckKT', d.TGBVCheckKT)
            setDateTimePicker('dp_nvCheckBD', d.TGNVCheckBD)
            setDateTimePicker('dp_nvCheckKT', d.TGNVCheckKT)
            setDateTimePicker('dp_dongHang', d.NgayGioDongHang)
            setDateTimePicker('dp_finishDongHang', d.NgayGioKTDongHang)
            setDateTimePicker('dp_xeDi', d.NgayGioXeDi)
...
function renderMaPKL() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length == 0) {
                showToast('warning', "Chưa có mã PKL xuất hàng")
                hideDisable()
                return;
            }
...
function hideDisable() {
    $(".banSo, .container, .romoc, .nameTX, .CMND,   .checkXe, .donghang, .finishDongHang ").prop("readonly", true);
}
function showDisable() {
    $(".banSo ,.container, .romoc, .nameTX, .CMND, .congty, .checkXe, .donghang, .finishDongHang ").prop("readonly", false);
}
...
$(".lookup-results1").on("click", "li", function () {
    if ($(this).hasClass("highlight")) {
        showToast('warning', "Phiếu này đã kiểm hàng")
        return;
    }
...
function GetCheckMakHo(mapkl, macont, $this) {
    $.ajax({
        url: `/api/BienBanLuuSeal/Get?Action=GetCheckMaKho&para1=${mapkl}&para2=${macont}&para3=a&para4=a&para5=a`,
        type: "Get",
        success: function (data) {
            if (data.length == 1) {
                eachCheckBoxLi()
                ...
                showToast('warning', "Hiện tại PKL này đang có 2 mã kho! Vui lòng luân chuyển về chung kho để được xuất hàng ")
...
$(".lookup-results1").on("click", ".checkLI", function () {
    if ($(this).prop("checked")) {
        let macont = $(this).closest("li").data("macont")
        let mapkl = $(this).closest("li").data("value")
        GetCheckMakHo(mapkl, macont, $(this))
    } else {
        eachCheckBoxLi()
        if ($(".container").val() == "")
            $(".container").val('Cont')
...
function renderTableMaHang() {
    if (filterData.length == 0) {
        showToast('warning', "Các phiếu đã được kiểm hàng.")
        hideDisable()
        $(".lookup-input2").val("")
        $("#bodyMahang").empty();
        return
        ...
        success: function (data) {
            ...
            let htmlFoter = `
                          <tr style="height:25px">
                            ...
                            <td class="TongSL" colspan="">${totalSoLuong}</td>
                             <td class="TongSK" colspan="">${totalSoKien}</td>
                            <td colspan="2"></td>
...
function showConfirmation(id_img) {
    ...
    $("#zoomImage").on("click", function () {
        ...
        if (imgSrc == null || imgSrc == undefined) {
            showToast('warning', "Vui lòng chụp hình hoặc chọn hình trước khi phóng to !")
            return;
        }
...
function eachDivImage(maphieubb) {
    ...
    arrDivImage.map(item => {
        if ($(`#${item}`).find('img[data-name="10"]').length > 0) {
            let imageNoiBo = $(`#${item}`).find("img").attr("src")
            let object = {
                MaBienBan: maphieubb,
                [item]: imageNoiBo,
                SoXe: index,
...
$(document).on("click", function (event) {
    if (!$(event.target).closest('.maSeal, .inputName,.input_image,.SoBooking,.selectseal').length) {
...
$('.selectseal').on('select2:open', function () {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))
        $(".list_kyten").addClass("active")
    $("html, body").animate({ scrollTop: $(document).height() }, 800);
});
            ...
            $.each(data, function (index, item) {
                ...
                $(".item_addImage").append(`
                 <div data-position="${count}"  class="image_other ${count}">
                  ...
                  <div  data-field="lst_AddImage${count}" onclick="handleImageClick(this)" data-name="${count}" id="lst_AddImage${count}" class="lst_AddImage">
...
$(".camera_imageOther").on("click", function () {
    ...
    if (divLength == 0) {
        showToast('warning', "Vui lòng chụp hình ")
        count--;
        countRow--;
        return;
    }
...
$(".item_addImage").on("click", ".lst_AddImage", function () {
    ...
    //if (inputImage == "") {
    //    showToast('warning', "Vui lòng nhập tiêu đề")
    //    return;
    //}
...
function foreachImageNoiBo(maphieu) {
    let maphieubb = maphieu;
    if (module == 2) maphieubb = maPhieu
    $(".image_other").each(function () {
        let $this = $(this);

        let imageNoiBo = $this.find("img").attr("src");

        // Không có ảnh
        if (!imageNoiBo) return true;

        // Không phải base64 => bỏ qua
        if (!imageNoiBo.startsWith("data:image/")) return true;
...
function saveImageOther(maphieu) {
    foreachImageNoiBo(maphieu)
}
...
$("#bodyKyTen").on('click', '.image_kyten', function () {
    ...
    if (hotenValue.trim() != "" && bophanValue.trim() != "") {
        ...
        showToast('warning', "Vui lòng nhập họ tên và bộ phận")
    }
});
function AddKyTen() {
    countKiten++;
...
function saveSignature() {
    ...
    if (!linkImageNoiBo) return;

    // Lấy ảnh từ canvas
    const imgData = canvas.toDataURL('image/png');

    // Xóa nội dung cũ
    linkImageNoiBo.innerHTML = '';

    // Thêm ảnh chữ ký
    const img = document.createElement('img');
    img.src = imgData;
    img.className = 'ky-img';
    let elementImageNoiBo = $(`<img data-name='${imgData}' class='linkhinh'>`).attr("src", imgData)
...
function saveKyTen(maphieu) {
    foreachTable(maphieu)
...
$("#xuatHangDetail").on("click", async function () {
    $.ajax({
        url: `/api/BienBanLuuSeal/Get?Action=GetMaPKLPhieu&para1=${maPhieu}&para2=A&para3=a&para4=a&para5=a`,
        type: "Get",
        success: function (data) {
            if (module == 2) {
                arrMaPKL_Cont = []
                arrMaPKL_Cont = data
                console.log(arrMaPKL_Cont)
            }
            let html = ``
            arrMaPKL_Cont.map(item => {
                html += `
...
function showConfirmationSeal(valueClear, defaultValues) {
    Swal.fire({
        ...
        cancelButtonColor: '#d33',
        showCloseButton: true,// Thêm nút đóng
        html: '<button  id="clear2" class="swal2-confirm swal2-styled" style="background-color: #f6c23e;display:none">Hủy</button>'
...
function showConfirmationSeal1(valueClear, defaultValues, $this) {
    Swal.fire({
        ...
        cancelButtonColor: '#d33',
        showCloseButton: true,// Thêm nút đóng
        html: '<button id="clear1" class="swal2-confirm swal2-styled" style="background-color: #f6c23e;display:none">Hủy</button>'
...
$(".listXe").on("click", function () {
    if (xeVC != 2) {
        showToast('warning', "Vui lòng chọn xe tải")
        return;
    }
    //$("#tbodyLists").empty()
    $(".input_popup").val("")
...
$("#tbodyLists").on("input", ".SLThungListInput", function () {
    ...
    if (totalSL > totalSoLuongTable) {
        showToast('warning', "Vui lòng không được nhập vượt tổng SLSP ")
...
$("#tbodyLists").on("input", ".SLSPistInput", function () {
    ...
    if (totalSThung > totalSoKienTable) {
        showToast('warning', "Vui lòng không được nhập vượt tổng số lương thùng ")
        $(this).val(value.slice(0, -1));
...
$("#theadLists ").on("click", ".listTenTXAll", function () {
    if ($tridListXe === "") {
        showToast('warning', "Vui lòng chọn dòng để thêm tên hoặc cmnd")
        return;
    }
    $(".popup4 ").show()
    $(".search_table1").val("")
...
$(".savelistTaxe").on("click", function () {
    ...
    $(".maSealSelect ").val(defaultValues).trigger('change');
    if ($("#tfootSL").text() == 0) {
        showToast('warning', "Vui lòng nhâp số lượng sản phẩm ")
        return;
    }
    if ($("#tfootSoKien").text() == 0) {
        showToast('warning', "Vui lòng nhâp số lượng thùng ")
        return;
    }
    $(".popup3").hide()
    ...
    arrDataPost.map(item => {
        if ($("#tbodyLists tr").length > 0) {
            ...
            let object = {
                ...
                CMND: $(".CMND").val(),
                SoXe: $(".banSo").val().toUpperCase(),
                SLSP: $("#bodyMahang").find(".TongSL").text() || 0,
                SLThung: $("#bodyMahang").find(".TongSK").text(),
                Module: 1
...
$(".maSealSelect").on("select2:select", function (e) {
    if (module == 2) {
        ...
        if (valseal == 1 && previousValue != null) {
            showToast('warning', "Seal này đã được sử dụng");
...
$("#btnKyTenDT").on('click', function () {

    if (module == 2 && maPhieu == "") {
        showToast('warning', "Vui lòng chọn phiếu trước");
        return;
    }
    $("#ifPhieuXuatKho").attr("src", `/ViTriKho/PhieuXuatKho?userName=${moduleWin == 5 ? user1 : user}&phieuPKL=${maPhieu}`);
    $("#modalPhieuXuatKho").modal('show');

});

$("#closeModalKyTen").on("click", function () {
    $("#modalKyTen").hide();
    $("#ifPhieuXuatKho").attr("src", "");
});