...
var dxDataGridKiemKe;
var checkIndex = 0;
var checkIndexNguoiGhiNhan = 0;
...
function createViewDxDataGridKiemKe(data) {
    dxDataGridKiemKe = $("#dxDataGridKiemKe").dxDataGrid({
        dataSource: data,
        width: '100%',
...
async function LoadPhieuKiem() {
    try {
        ...
        const url = `/api/DangkyVatTu/Get?Action=GetDanhSachPhieuKiemKe&Para1=${tuNgay}&Para2=${denNgay}&Para3=${locTrangThai}&Para4=${1}`
...
async function LoadDanhSachKK() {
    try {
        ...
        const url = `/api/DangkyVatTu/Get?Action=GetPhieuKiemKeXN&Para1=${phieuKiemKe}&Para2=${1}`
        const response = await fetch(url);
        const data = await response.json();
