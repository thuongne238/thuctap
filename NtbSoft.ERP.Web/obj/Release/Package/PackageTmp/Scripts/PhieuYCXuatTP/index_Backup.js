...
async function exportPhieuXuatKhoExcel(data) {
    ...
    Object.entries(grouped).forEach(([phieu, rows], phieuIndex) => {
        ...
        // Xác định URL ảnh chữ ký "Người lập" (SignNgDK)
        let signImgUrl = null;
    ...
    //XLSX.writeFile(wb, `PhieuXuatKho_${dateStr}.xlsx`);
    //const ws = XLSX.utils.aoa_to_sheet(allRows);
    //ws["!merges"] = merges;
    //ws["!cols"] = colWidths;
    ...
    //// ── Xuất buffer bằng SheetJS rồi nạp qua ExcelJS để chèn ảnh ──
    ...
    //    exWs.addImage(imgId, {
    //        tl: { col: anchor.col, row: anchor.row }, // 0-based, top-left của ô
    //        ext: { width: 90, height: 45 }             // kích thước ảnh (px)
    //    });
    //}
    ...
    // Đặt chiều cao cho 3 dòng trống chứa chữ ký (mỗi dòng 20pt)
    for (const anchor of signatureAnchors) {
        for (let i = 0; i < 3; i++) {
            const rowObj = exWs.getRow(anchor.row + i + 1); // ExcelJS dùng 1-based
            rowObj.height = 20; // point
        }
    }
    ...
    for (const anchor of signatureAnchors) {
        ...
        // Lấy chiều rộng thực tế của cột D (cột 4) và E (cột 5) sau khi load
        const colD = exWs.getColumn(4);
        const colE = exWs.getColumn(5);
        const wD = colD?.width || 11.55;  // fallback về giá trị đã định nghĩa
        const wE = colE?.width || 17.22;

        // Công thức quy đổi character width -> pixel của Excel: px = width * 7 + 5
        const totalWidthPx = (wD * 7 + 5) + (wE * 7 + 5);
        const imgWidth = 90;
        const offsetColEMU = Math.round((totalWidthPx / 2 - imgWidth / 2) * 9525);

        // Chiều cao 3 dòng (đã set 20pt mỗi dòng)
        const rowHeightPt = 20;
        const totalRowPx = rowHeightPt * 3 * 96 / 72; // 20*3=60pt -> 60*96/72=80px
        const imgHeight = 45;
        const offsetRowEMU = Math.round((totalRowPx / 2 - imgHeight / 2) * 9525);
        ...
        exWs.addImage(imgId, {
            tl: {
                col: 3,                          // cột D
                row: anchor.row,                 // dòng đầu tiên trong 3 dòng trống
                colOff: offsetColEMU,
                rowOff: offsetRowEMU
            },
    ...
    const blob = new Blob([outBuffer], { type: "application/octet-stream" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob); // sửa lỗi chính tả
    link.download = `PhieuXuatKho_${dateStr}.xlsx`;
    link.click();
    URL.revokeObjectURL(link.href);
}
