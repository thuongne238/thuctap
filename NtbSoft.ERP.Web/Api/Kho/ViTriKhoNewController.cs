...
{
    //[HMACAuthentication]
    [RoutePrefix("api/ViTriKhoNew")]
    public class ViTriKhoNewController : ApiController
    {
        [HttpGet]
        [Route("Get")]
        public DataTable Get(string action, string Para1, string Para2, string Para3, string Para4, string Para5, string Para6)
        {
            return new ViTriKhoNewModel().Get(action, Para1, Para2, Para3, Para4, Para5, Para6);
        }
        [HttpPost]
        [Route("Post")]
        public string Post(string action,DataTable dt)
        {
            return new ViTriKhoNewModel().Post(dt, action, "@TypeViTriKho");
        }
        [HttpPost]
        [Route("PostKho")]
        public string PostKho(string action, DataTable dt)
        {
            return new ViTriKhoNewModel().Post(dt, action, "@TypeKhaiBaoKho");
        }
    }
}
