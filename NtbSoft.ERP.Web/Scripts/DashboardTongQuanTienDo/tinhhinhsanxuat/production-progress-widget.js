...
(function (root, factory) {
        ...
        if (typeof this.options.dataSource === 'string') {
            $.ajax({
                ...
                if (res && res.success && Array.isArray(res.data) && res.data.length > 0) {
                    ...
                    self.rawData = generateGarmentProductionData(self.timePeriod);
                }
                self.applyFilter();
            }).fail(function () {
                self.rawData = generateGarmentProductionData(self.timePeriod);
            ...
            this.rawData = this.options.dataSource;
            this.applyFilter();
        } else {
            this.rawData = generateGarmentProductionData(this.timePeriod);
            this.applyFilter();
        }
    };
    ...
    // =========================================================================
    // 2. BÊN PHẢI: BIỂU ĐỒ TRÒN (PIE CHART TIẾN ĐỘ SẢN XUẤT)
    // - Hình tròn đặc
    // - Tên PO được GHI RÕ Ở BÊN TRONG Ô (INSIDE SLICE), KHÔNG Ở NGOÀI
    // - Rê chuột vào PO hiện Tooltip Card kiểu ảnh thứ 2 theo nghiệp vụ Sản Xuất
    // - Khi chọn 1 PO: Biểu đồ tròn hiển thị phân bổ công đoạn của chính PO đó
    // =========================================================================
    ProductionProgressWidget.prototype.renderPieChart = function () {
        ...
        if (this.selectedPO) {
            // -------------------------------------------------------------------
            // TRƯỜNG HỢP 1: ĐÃ CHỌN 1 PO -> Biểu đồ tròn thể hiện phân bổ công đoạn SX của PO
            // -------------------------------------------------------------------
            const po = this.selectedPO;
            $('#' + this.id + '_pie_title').html(`TIẾN ĐỘ PO: <b>${po.po}</b> (${po.progress}%)`);
            $backBtn.show();
            ...
            pieData = [
                {
                    po: po.po,
                    sliceName: `${po.po} (Đã SX)`,
                    qty: doneQty > 0 ? doneQty : 1,
                    ...
                    endDate: po.endDate,
                    etd: po.etd,
                    color: getProgressColor(po.progress)
                },
                {
                    po: po.po,
                    sliceName: 'Còn lại',
            ...
            // -------------------------------------------------------------------
            // TRƯỜNG HỢP 2: TẤT CẢ PO (MỖI LÁT BÁNH LÀ 1 PO - TÊN PO BÊN TRONG Ô)
            // -------------------------------------------------------------------
            $('#' + this.id + '_pie_title').text('PHÂN BỔ SẢN LƯỢNG CÁC PO');
            $backBtn.hide();

            pieData = this.filteredData.map(item => ({
                po: item.po,
                sliceName: item.po,
                qty: item.qty || 1,
        ...
        // Ưu tiên dùng DevExtreme dxPieChart
        if (typeof DevExpress !== 'undefined' && DevExpress.ui && DevExpress.ui.dxPieChart) {
            try {
                $(pieElem).empty();

                const pieOptions = {
                    dataSource: pieData,
                    type: 'pie', // BIỂU ĐỒ TRÒN ĐẶC (PIE CHART)
                    size: {
                        height: 330,
                        width: 360
                    },
                    minSegmentSize: 18, // Giúp lát bánh nhỏ vẫn hiển thị đủ chữ bên trong
                    series: [{
                        argumentField: 'sliceName',
                        valueField: 'qty',
                        label: {
                            visible: true,
                            position: 'inside', // ĐƯỢC GHI RÕ Ở BÊN TRONG Ô, KHÔNG Ở NGOÀI
                            radialOffset: 0,
                            ...
                            customizeText: function (arg) {
                                // Ghi rõ tên PO bên trong ô
                                return arg.point.data.po;
                            }
                        }
                    }],
                    ...
                    tooltip: {
                        ...
                        customizeTooltip: function (arg) {
                            ...
                            return {
                                html: `
                  <div style="font-family: 'Inter', sans-serif; font-size: 12px; line-height: 1.65; color: #334155; text-align: left; min-width: 200px;">
                    <div style="font-size: 13.5px; font-weight: 800; color: #0f172a; margin-bottom: 4px; border-bottom: 1px solid #f1f5f9; padding-bottom: 3px;">PO: ${data.po}</div>
                    <div>Mã hàng: <strong>${data.orderCode || '-'}</strong></div>
                    <div>Khách hàng: <strong>${data.customer || '-'}</strong></div>
                    <div>Số lượng SX: <strong>${formatNumber(data.qty)} pcs</strong></div>
                    <div>Thời gian SX: <strong>${data.startDate || data.etd} - ${data.endDate || data.etd}</strong></div>
                    <div>Tiến độ sản xuất: <span style="font-weight: 800; color: ${data.color};">${data.progress}%</span> <span style="font-size: 11px; color: #64748b;">${detailNote}</span></div>
                  </div>
                `
                            };
                        }
                    ...
                    onPointHoverChanged: function (e) {
                        ...
                        if (point.isHovered()) {
                            self.highlightTableRow(point.data.po);
                        } else {
                            self.unhighlightTableRow();
                        }
                    },
                    onPointClick: function (e) {
                        const data = e.target.data;
                        if (data && data.po) {
                            const match = self.filteredData.find(x => x.po === data.po);
                            if (match) self.selectPO(match);
                        }
                    }
                };

                this.pieInstance = $(pieElem).dxPieChart(pieOptions).dxPieChart('instance');

                setTimeout(function () {
                    if ($(pieElem).find('svg').length === 0) {
                        self.renderSvgPieChart(pieElem, pieData);
                    }
                }, 80);
                return;
            } catch (err) {
                console.warn('[ProductionProgressWidget] dxPieChart fallback to SVG:', err);
            }
        }

        // BỘ VẼ DỰ PHÒNG SVG PIE CHART TRÒN ĐẶC (GHI TÊN PO BÊN TRONG Ô & HOVER TOOLTIP CARD)
        this.renderSvgPieChart(pieElem, pieData);
    };

    ProductionProgressWidget.prototype.renderSvgPieChart = function (container, data) {
        const self = this;
        const total = data.reduce((s, d) => s + (d.qty || 1), 0) || 1;
        const width = 340;
        const height = 320;
        const radius = 120;
        const cx = width / 2;
        ...
        data.forEach(slice => {
            const sliceAngle = (slice.qty / total) * 2 * Math.PI;
        ...
        <path d="M ${cx} ${cy} L ${x1} ${y1} A ${radius} ${radius} 0 ${largeArc} 1 ${x2} ${y2} Z"
              fill="${color}" stroke="#ffffff" stroke-width="2.5" class="ppw-svg-slice"
              data-po="${slice.po}" style="cursor: pointer; transition: transform 0.15s, opacity 0.15s;">
        </path>
      `;

            // GHI RÕ TÊN PO Ở BÊN TRONG Ô (INSIDE SLICE)
            ...
            labels += `
        <text x="${lx}" y="${ly}" font-size="11" font-weight="700" text-anchor="middle" dominant-baseline="central"
              fill="#ffffff" style="pointer-events: none; text-shadow: 0 1px 3px rgba(0,0,0,0.7);">
          ${slice.po}
        </text>
      `;
    ...
    `;
        ...
        $(container).find('.ppw-svg-slice')
            .on('mouseenter', function (e) {
                const poKey = $(this).data('po');
                const match = self.filteredData.find(x => x.po === poKey);
                if (match) {
                    self.showCustomTooltip(e, match);
                    self.highlightTableRow(poKey);
                    $(this).attr('stroke', '#ffffff').attr('stroke-width', '4').css('opacity', '0.92');
                }
            })
            .on('mousemove', function (e) {
                const poKey = $(this).data('po');
            ...
            .on('mouseleave', function () {
                self.hideCustomTooltip();
                self.unhighlightTableRow();
                $(this).attr('stroke', '#ffffff').attr('stroke-width', '2.5').css('opacity', '1');
            })
            .on('click', function () {
                const poKey = $(this).data('po');
                const match = self.filteredData.find(x => x.po === poKey);
